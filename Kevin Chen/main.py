...
def agent(obs, config):
    ...
    if len(me.ships) <= (config.episodeSteps - turn) / 50 and len(me.shipyards) > 0 and me.halite >= config.spawnCost:
        me.shipyards[0].next_action = ShipyardAction.SPAWN

    # If there are no shipyards, convert first ship into shipyard.
    if len(me.shipyards) == 0 and len(me.ships) > 0 and me.halite >= config.convertCost:
